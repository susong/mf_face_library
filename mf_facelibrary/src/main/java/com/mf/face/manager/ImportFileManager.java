...
import com.mf.face.listener.OnImportListener;
import com.mf.face.model.ImportFeatureResult;
...
public class ImportFileManager {
    ...
    private Future mImportImageFuture;
    private ExecutorService mImportImageExecutorService;
    ...
    private ImportFileManager() {
        if (mImportImageExecutorService == null) {
            mImportImageExecutorService = Executors.newSingleThreadExecutor();
        }
    }
    ...
    public void batchImport() {
        ...
        File batchImportDir = FileUtils.getFaceImportDir();
        // 2、遍历该目录下的所有文件
    ...
    private void asyncImport(final File[] picFiles) {
        if (mImportImageFuture != null && !mImportImageFuture.isDone()) {
            return;
        }
        ...
        mImportImageFuture = mImportImageExecutorService.submit(new Runnable() {
            @Override
            public void run() {
                try {
                    if (picFiles == null || picFiles.length == 0) {
                    ...
                    for (int i = 0; i < picFiles.length; i++) {
                        ...
                        LogUtils.e(TAG, "i = " + i + ", picName = " + picName);
                        // 4、判断图片后缀
                        if (!picName.endsWith(".jpg") && !picName.endsWith(".png") &&
                                !picName.endsWith(".PNG") && !picName.endsWith(".JPG")) {
                            LogUtils.e(TAG, "图片后缀不满足要求");
                            FileUtils.saveFile(picFiles[i], FaceConstant.FaceImportFailDir,
                                    name + "_1");
                            mFinishCount++;
                            mFailCount++;
                            // 更新进度
...
//                                 更新进度
                        ...
                        if (bitmap == null) {
                            ...
                            mFinishCount++;
                            mFailCount++;
                            BitmapUtils.saveRgbBitmap(bitmap, FaceConstant.FaceImportFailDir,
                                    name + "_2");
                            // 更新进度
                            updateProgress(mTotalCount, mSuccessCount, mFailCount,
                                    ((float) mFinishCount / (float) mTotalCount));
                        ...
                        LogUtils.i(TAG, "live_photo = " + result.getResult());
                        if (result.getResult() == 128) {
                            ...
                            if (importDBSuccess) {
                                ...
                                BitmapUtils.saveRgbBitmap(bitmap1, FaceConstant.FaceImportFailDir,
                                        name + "_10");
                            }
                        } else {
                            LogUtils.e(TAG, picName + " 错误码：" + result.getResult());
                            BitmapUtils.saveRgbBitmap(bitmap1, FaceConstant.FaceImportFailDir,
     ...
     * 提取特征值
     */
    private ImportFeatureResult getFeature(Bitmap bitmap, byte[] feature, BDFaceSDKCommon.FeatureType featureType) {
        if (bitmap == null) {
     ...
     * @return
     */
    private int onQualityCheck(FaceInfo faceInfo) {
     ...
     */
    public void release() {
        if (mImportImageFuture != null && !mImportImageFuture.isCancelled()) {
            mImportImageFuture.cancel(true);
            mImportImageFuture = null;
        }

        if (mImportImageExecutorService != null) {
            mImportImageExecutorService.shutdown();
            mImportImageExecutorService = null;
        }
    }
}
