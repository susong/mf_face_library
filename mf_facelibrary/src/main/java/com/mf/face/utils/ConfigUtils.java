...
public class ConfigUtils {
    ...
    public static boolean isConfigExist(){
        folder = PathUtils.getExternalStoragePath() + GlobalConstants.APP_BASE_PATH + File.separator + "config";
        filePath = folder + File.separator + "faceConfig.json";
        System.out.println(">>>>>>>faceConfig:" + filePath);
...
//            SingleBaseConfig.getBaseConfig().setThreshold(jsonObject.getInt("threshold"));
            ...
            e.printStackTrace();
            LogUtils.e("facesdk", "文件内容异常,请检测是否规范");
            return false;
        }
    }
...
//            int radius = (int) jsonObject.get("radius");
            ...
            e.printStackTrace();
            LogUtils.e("facesdk", "文件内容格式异常,请检测是否规范");
            return false;
        }
